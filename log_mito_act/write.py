...
for each in models:
    ...
    f.write('	else:\n')
    f.write('		parpc = model_solver.yobs[-1][6]/(model_solver.yobs[-1][1] + model_solver.yobs[-1][6])\n')
    f.write('           if (parpc > 0.0) and (parpc < 1.00000001):\n')
    f.write('			print log(model_solver.yobs[-1][6]/(model_solver.yobs[-1][1] + model_solver.yobs[-1][6])), point\n')
    f.write('			return [\'sim\', log(model_solver.yobs[-1][6]/(model_solver.yobs[-1][1] + model_solver.yobs[-1][6]))]\n\n\n')
    f.write('		else:\n')
    f.write('			return [\'fail\', -10000.0]\n')
    f.write('n_params = 0\n')
    ...
    f.write('counts = [0, 0]\n')
    f.write('pymultinest.run(loglike, prior, n_params, evidence_tolerance=0.0001, n_live_points=16000, log_zero=-1e3, sampling_efficiency=0.3, outputfiles_basename=\'/path/to/log_mito_act/output/' + each.split('_')[1][:-3] +  '/\', resume = False, verbose = False, counts=counts)\n\n')
