...
for each in models:
    ...
    f.write('			ratio_m = sum_m/(sum_c + sum_m)\n')
    f.write('		sum_mp += ratio_m*(every[8] - cParp)\n')
    f.write('		sum_cp += (1 - ratio_m)*(every[8] - cParp)\n')
    f.write('		cParp = deepcopy(every[8])\n')
    f.write('	if failed:\n')
    f.write('		return [\'fail\', -10000.0]\n')
    f.write('	else:\n')
    f.write('		m_cParp_proportion = sum_mp/model.parameters[61].value\n')
    f.write('		if (m_cParp_proportion > 0.0) and (m_cParp_proportion < 1.00000001):\n')
    f.write('			print log(m_cParp_proportion), point\n')
    f.write('			return [\'sim\', log(m_cParp_proportion)]\n\n')
    f.write('		else:\n')
    f.write('			return [\'fail\', -10000.0]\n') 
    ...
    f.write('counts = [0, 0]\n')
    f.write('pymultinest.run(loglike, prior, n_params, evidence_tolerance=0.0001, n_live_points=4000, log_zero=-1e3, sampling_efficiency=0.3, outputfiles_basename=\'/path/to/flux_mito/output/' + each.split('_')[1][:-3] +  '/\', resume = False, verbose = False, counts=counts)\n\n')
