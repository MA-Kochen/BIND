...
for each in models:
    ...
    f.write('	else:\n')
    f.write('		parpc = model_solver.yobs[-1][12]/(model_solver.yobs[-1][2] + model_solver.yobs[-1][12])\n')
    ...
    f.write('counts = [0, 0]\n')
    f.write('pymultinest.run(loglike, prior, n_params, evidence_tolerance=0.0001, n_live_points=16000, log_zero=-1e3, sampling_efficiency=0.3, outputfiles_basename=\'/path/to/log_caspase/output/' + each.split('_')[1][:-3] +  '/\', resume = False, verbose = False, counts=counts)\n\n')
