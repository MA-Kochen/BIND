...
for each in models:
    ...
    f.write('from pysb.integrate import ScipyOdeSimulator\n')
    f.write('import csv\n')
    ...
    f.write('model_solver = ScipyOdeSimulator(model, tspan=time, integrator=\'vode\', integrator_options={\'atol\': 1e-12, \'rtol\': 1e-12})\n\n\n')

    f.write('def prior(cube, ndim, nparams):\n\n')

    f.write('	for k, every in enumerate(model.parameters):\n')

    f.write('		if every.name[-3:] == \'2df\':\n')
    f.write('			cube[k] = cube[k]*4 - 8\n')
    f.write('		if every.name[-3:] == \'1dr\':\n')
    f.write('			cube[k] = cube[k]*4 - 8\n')

    f.write('		if every.name[-3:] == \'2xf\':\n')
    f.write('			cube[k] = cube[k]*4 - 8\n')
    f.write('		if every.name[-3:] == \'1xr\':\n')
    f.write('			cube[k] = cube[k]*4 - 7\n')
    ...
    f.write('postfixes = [\'2df\', \'1dr\', \'2xf\', \'1xr\', \'1kf\', \'2kf\', \'1kr\', \'1kc\']\n\n\n')
    ...
    f.write('	sum_cp = 0.0\n')
    f.write('	sum_mp = 0.0\n')
    f.write('	sum_total = 0.0\n')
    f.write('	cParp = 0.0\n\n')

    f.write('	for every in model_solver.run(param_values=point).species:\n')
    f.write('		for thing in every:\n')
    f.write('			if thing <= -0.00000001 or np.isnan(thing):\n')
    f.write('				failed = True\n')
    f.write('		if failed:\n')
    f.write('			break\n')
    f.write('		sum_total += every[8] - cParp\n')
    f.write('		cParp = deepcopy(every[8])\n')
    f.write('	if failed:\n')
    f.write('		return [\'fail\', -10000.0]\n')
    f.write('	else:\n')
    f.write('		total_cParp = sum_total/model.parameters[61].value\n')
    f.write('		if (total_cParp > 0.0) and (total_cParp < 1.00000001):\n')
    f.write('			print log(total_cParp), point\n')
    f.write('			return [\'sim\', log(total_cParp)]\n\n')
    f.write('		else:\n')
    f.write('			return [\'fail\', -10000.0]\n')
    ...
    f.write('	if lotsa.name[-3:] == \'2xf\':\n')
    f.write('		n_params += 1\n')
    f.write('	if lotsa.name[-3:] == \'1xr\':\n')
    f.write('		n_params += 1\n')
    f.write('	if lotsa.name[-3:] == \'2df\':\n')
    f.write('		n_params += 1\n')
    f.write('	if lotsa.name[-3:] == \'1dr\':\n')
    f.write('		n_params += 1\n')
    ...
    f.write('counts = [0, 0]\n')
    f.write('pymultinest.run(loglike, prior, n_params, evidence_tolerance=0.0001, n_live_points=4000, log_zero=-1e3, sampling_efficiency=0.3, outputfiles_basename=\'/path/to/flux_combined_high_binding/output/' + each.split('_')[1][:-3] +  '/\', resume = False, verbose = False, counts=counts)\n\n')
