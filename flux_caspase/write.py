...
for each in models:
    ...
    f.write('		if failed:\n')
    f.write('			break\n')
    f.write('		sum_c += max(every[39], 0,0)*model.parameters[53].value\n')
    f.write('		sum_m += max(every[30], 0,0)*model.parameters[18].value\n')
    f.write('		ratio_c = 0.0\n')
    f.write('		if sum_c != 0.0 or sum_m != 0.0:\n')
    f.write('			ratio_c = sum_c/(sum_c + sum_m)\n')
    f.write('		sum_cp += ratio_c*(every[8] - cParp)\n')
    f.write('		sum_mp += (1 - ratio_c)*(every[8] - cParp)\n')
    f.write('		cParp = deepcopy(every[8])\n')
    f.write('	if failed:\n')
    f.write('		return [\'fail\', -10000.0]\n')
    f.write('	else:\n')
    f.write('		cc_cParp_proportion = sum_cp/model.parameters[61].value\n')
    f.write('		if (cc_cParp_proportion > 0.0) and (cc_cParp_proportion < 1.00000001):\n')
    f.write('			print log(cc_cParp_proportion), point\n')
    f.write('			return [\'sim\', log(cc_cParp_proportion)]\n\n')
    f.write('		else:\n')
    f.write('			return [\'fail\', -10000.0]\n')
    ...
    f.write('counts = [0, 0]\n')
    f.write('pymultinest.run(loglike, prior, n_params, evidence_tolerance=0.0001, n_live_points=4000, log_zero=-1e3, sampling_efficiency=0.3, outputfiles_basename=\'/path/to/flux_caspase/output/' + each.split('_')[1][:-3] +  '/\', resume = False, verbose = False, counts=counts)\n\n')
